...
#endif
    if (range.location>self.length) {
        YRWarningLog(@"NSString stringByReplacingCharactersInRangeSafe range out of bounds,rang (%lu,%lu) out of range (%d,%lu),string=%@",(unsigned long)range.location,(unsigned long)range.length,0,(unsigned long)self.length,self);
        return nil;
    }
    if (range.location+range.length>self.length) {
        YRWarningLog(@"NSString stringByReplacingCharactersInRangeSafe range out of bounds,rang (%lu,%lu) out of range (%d,%lu),string=%@",(unsigned long)range.location,(unsigned long)range.length,0,(unsigned long)self.length,self);
